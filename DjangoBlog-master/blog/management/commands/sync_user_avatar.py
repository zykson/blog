...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write('STARTSYNCING{count}Useravatars'.format(count=len(users)))
        for u in users:
            self.stdout.write('STARTSYNCING:{id}'.format(id=u.nikename))
            url = u.picture
            url = save_user_avatar(url)
            if url:
                self.stdout.write(
                    'ENDSYNC:{id}.url:{url}'.format(
                        id=u.nikename, url=url))
                u.picture = url
                u.save()
        self.stdout.write('ENDSYNC')
